...
TEspacing = 0.02;
    ...
    void printFile() {
        ...
        D = 1.2;      /* Distanza dal muro*/
        AoA = 4;  /* Angolo di attacco in deg*/
        ...
        BL_ratio = 1 + (hpp - BL_size)/BL_thickness;  /*rapporto spessori BL profilo*/
                ...
                /* STRUTTURATA MURO*/
        hrat = 5; /* Rapporto tra h sul pezzo centrale del muro e h sul profilo*/
        BL = 0.03;   /* Spessore layer vicino al muro*/
        ...
        if (output.is_open()){
            ...
            for (int iPoint = 0; iPoint < nPoints; iPoint++) {
                ...
                output << "Point(" << iPoint+1 << ") = {" << coordinates[iPoint][0]
                       << ", " << coordinates[iPoint][1] - vtrasl<< ", 0.0, " << h[iPoint] <<"*h};\n";
            }
            ...
            if (isProfile) {
                ...
                output << "Point(" << nPoints + 15 << ") = {1.1, " << (RotPoint-1)*sin(AoA*3.14/180) - vtrasl<<", 0,2*h};\n";      
                output << "Point(" << nPoints + 16 << ") = {1.2, " << (RotPoint-1)*sin(AoA*3.14/180) - vtrasl<<", 0,3*h};\n";  
                output << "Point(" << nPoints + 17 << ") = {1.3, " << (RotPoint-1)*sin(AoA*3.14/180) - vtrasl<<", 0,3*h};\n";      
                output << "Point(" << nPoints + 18 << ") = {1.5, " << (RotPoint-1)*sin(AoA*3.14/180) - vtrasl<<", 0,4*h};\n";      
		    ...
		    output << "Spline(4) = {" << LOid + 1 << ":" << nPoints-1 << ", 1};\n";*/
                ...
                */
                ...
                output << "Rotate {{0, 0, -1}, {" << RotPoint <<", " << -vtrasl<< ", 0}, Pi * AoA / 180} \n {Curve{3};}\n";
