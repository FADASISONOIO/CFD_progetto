...
fileName = "C:/Bash/SU2/GIT_CFD/CFD_progetto/Utils/createGEO/txt_dat/main.dat";*/
    ...
    void printFile() {
        ...
        ofstream output("mesh_upper.geo");
        ...
        if (output.is_open()){
		    ...
		    output << "Spline(4) = {" << LOid + 1 << ":" << nPoints-1 << ", 1};\n";*/
                ...
                */
                ...
                output << "Physical Line(\"UP_INTERFACE\") = {8};\n";
                if (bluntTE)
        ...
        ofstream output2("mesh_lower.geo");
        ...
        if (output2.is_open()){
            ...
            if (isProfile) {
                ...
                output2 << "BL = 0.3;\n";   /* Spessore layer vicino al muro*/
                output2 << "AoA = 0.3;\n";  /* Angolo di attacco in deg*/
                output2 << "BL_size  = 0.00005;\n";  /*Size prima cella BL*/
                output2 << "BL_thickness = 0.01;\n"; /*Spessore BL profilo*/
                output2 << "BL_ratio = 1.1;\n";  /*rapporto spessori BL profilo*/
                output2 << "bumpW = 7;\n";  /*infittimento bounday muro in stramwise*/
                output2 << "progW = 1.1;\n";  /*infittimento bounday muro in wall normal*/
                output2 << "numW = 200;\n";  /*n celle stramwise muro*/
            }
            ...
            if (isProfile) {
                ...
                output2 << "\n // farfield\n";
                output2 << "Point(" << nPoints + 7 << ") = {R+0.5,  -D+BL, 0,1/5*H};\n";
                output2 << "Point(" << nPoints + 8 << ") = {1, -D+BL, 0,1/5*H};\n";
                output2 << "Point(" << nPoints + 9 << ") = {0, -D+BL, 0,1/5*H};\n";
                output2 << "Point(" << nPoints + 10 << ") = {0.5-R, -D+BL, 0,1/5*H};\n";
                output2 << "Point(" << nPoints + 11 << ") = {R+0.5,  -D, 0,1/5*H};\n";    
                output2 << "Point(" << nPoints + 12 << ") = {0.5-R, -D, 0,1/5*H};\n";           


                if (bluntTE)
                    output2 << "Line(9) = {" << nPoints << ",1};\n";

                output2 << "\n // farfield\n";

                output2 << "Line(8) = {" << nPoints + 7 << "," << nPoints + 10 << "};\n";
                output2 << "Line(9) = {" << nPoints + 8 << "," << nPoints + 9 << "};\n";
                output2 << "Line(10) = {" << nPoints + 9 << "," << nPoints + 10 << "};\n";
                output2 << "Line(12) = {" << nPoints + 11 << "," << nPoints + 12 << "};\n";
                output2 << "Line(13) = {" << nPoints + 7 << "," << nPoints + 11 << "};\n";
                output2 << "Line(14) = {" << nPoints + 12 << "," << nPoints + 10 << "};\n";
                /*output << "Line(12) = {" << LEid +1 << "," << nPoints + 5 << "};\n"; */
                /*output << "Line(13) = {" << nPoints + 3 << ",1};\n";*/ 
                ...
                output << "Transfinite Line{" << 9 << "} = npP Using Progression 1;\n";
                */
                output2 << "Transfinite Line{" << 8 << " , " << 12 << "} = numW Using Bump bumpW;\n";
                output2 << "Transfinite Line{" << 13 << "} =  20 Using Progression 1/progW;\n";
                output2 << "Transfinite Line{" << 14 << "} =  20 Using Progression progW;\n";
                
                output2 << "\n\n// =====================================LOOPS\n\n";

                output2 << "Line Loop(1) = {-8,13,12,14};\n";



                output2 << "\n\n// =====================================SURFS\n\n";
                output2 << "Plane Surface(1) = {1};\n";
                output2 << "Transfinite Surface{1};\n Recombine Surface{1};\n";

                // ==================================BOUNDARY LAYER

                output2 << "Physical Surface(1) = {1};\n";
                output2 << "Physical Line(\"FARFIELD\") = {14,13};\n";
                output2 << "Physical Line(\"WALL\") = {12};\n";
                output2 << "Physical Line(\"LOWER_INTERFACE\") = {8};\n";

            } else {

                output2 << "\n // symmetry\n";
                output2 << "Point(" << nPoints+2 << ") = {" << coordinates[0][0] << ", 0, 0, " << LEspacing << "*h};\n";
                output2 << "Point(" << nPoints+3 << ") = {" << coordinates[LEid][0] << ", 0, 0, " << 1 << "*h};\n";
                output2 << "Point(" << nPoints+4 << ") = {" << coordinates[nPoints-1][0] << ", 0, 0, " << TEspacing << "*h};\n";;


                output2 << "\n\n// =====================================CURVES\n\n";

                output2 << "Spline(1) = {1:" << LEid+1 << "};\n";
                output2 << "Spline(2) = {"<< LEid+1 << ":" << nPoints << "};\n";

                output2 << "Line(3) = {" << nPoints+2 << ", " << nPoints+3 << "};\n";
                output2 << "Line(4) = {" << nPoints+3 << ", " << nPoints+4 << "};\n";
                output2 << "Line(5) = {" << nPoints+2 << ", " << 1 << "};\n";
                output2 << "Line(6) = {" << nPoints+4 << ", " << nPoints << "};\n";

                output2 << "\n\n// =====================================LOOPS\n\n";

                output2 << "Line Loop(1) = {1, 2, -6, -3, -4, 5};\n";

                output2 << "\n\n// =====================================SURFS\n\n";
                output2 << "Plane Surface(1) = {1};\n\n";

                output2 << "Physical Surface(1) = {1};\n";
                output2 << "Physical Line(\"WALL\") = {1,2};\n";
                output2 << "Physical Line(\"INLET\") = {5};\n";
                output2 << "Physical Line(\"OUTLET\") = {6};\n";
                output2 << "Physical Line(\"SYMMETRY\") = {3,4};\n";
