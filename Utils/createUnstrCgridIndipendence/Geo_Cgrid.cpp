...
public:
    string fileName;
    double LEspacing,TEspacing,wakeref,wall_BL_size,hpp,H,R,H_inner,R_inner,D,BL,AoA,BL_size,BL_thickness,BL_ratio,progsWfwd,progsWaft,prognW,numWfwd,numWaft,nblW,hrat;
    ...
    void printFile() {
        ...
        if (output.is_open()){
            ...
            if (isProfile) {
                ...
                output << "H = " << H << ";\n";
                output << "R = " << R << ";\n";      /* Raggio C*/
                output << "H_inner = " << H_inner << ";\n";
                output << "R_inner = " << R_inner << ";\n";      /* Raggio C*/
                output << "wakeref = " << wakeref << ";\n";
                /*PROFILO*/
                output << "D = " << D << ";\n";      /* Distanza dal muro*/
            ...
            if (isProfile) {
                ...
                int innerpoints = floor(3.14/(2*asin(H_inner/(2*R_inner))*(1+1/wakeref)));
                
                
                int nP = 1;

                double dtheta_ratio = 1 + (H_inner - H_inner/(wakeref))/(3.14*R_inner/2);
                double dtheta = H_inner/(R_inner*wakeref);
                double theta = 0;
                while (theta < 3.14/2) {
                    dtheta = dtheta * dtheta_ratio;
                    theta = theta + dtheta;   
                output << "Point(" << nPoints + 16 + nP << ") = {" <<0.5+ R_inner*cos(theta) << " , " << R_inner*sin(theta)<<",0," << 2*R_inner*sin(dtheta/2) <<"};\n";
                nP = nP+1;
            }   
            while (theta < 3.14) {
                    dtheta = 2*asin(H_inner/(2*R_inner));
                    theta = theta + dtheta;   
                output << "Point(" << nPoints + 16 + nP << ") = {" <<0.5+ R_inner*cos(theta) << " , " << R_inner*sin(theta)<<",0," << 2*R_inner*sin(dtheta/2) <<"};\n";
                nP = nP+1;
            }   
		    ...
		    output << "Spline(4) = {" << LOid + 1 << ":" << nPoints-1 << ", 1};\n";*/
                ...
                */
                ...
                dtheta = H_inner/(R_inner*wakeref);
                nP = 1;
                theta= 0;
                while (theta < 3.14/2) {
                    dtheta = dtheta * dtheta_ratio;
                    theta = theta + dtheta;   
                output << "Point{" << nPoints + 16 +nP<< "} In Surface{" << 1 <<"};\n";
                nP = nP+1;
            }   
